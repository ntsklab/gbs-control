...
| `options.h` | `components/gbs_control/include/options.h` | **変更なし** (そのままコピー) |
...
### 3.3.1 ESP-IDF 側で管理する拡張ファイル（upstream から上書きしない）

| ファイル | 役割 |
|---|---|
| `components/gbs_control/src/geometry_buttons.cpp` | D-Pad入力タスク、モードトグル、モードLED表示 |
| `components/gbs_control/include/geometry_buttons.h` | 上記の公開ヘッダ |
| `components/gbs_control/src/shell.cpp` | `set input <r-encoder|d-pad>` と Tab補完 |
| `main/main.cpp` | 入力モード初期化を `gbs_setup()` 側へ一本化 |
...
upstream の `gbs-control.ino` → `gbs_control.cpp` に適用する変更を以下にカテゴリ別に示す。
入力モード切替（R/E ⇔ D-Pad）は `pin_config.h` のビルド時定数 `PINCFG_USE_ROTARY_ENCODER` で
制御する。`options.h` や `preferencesv2.txt` への変更は不要。
...
### 4.14 入力モード抽象化（R/E と D-Pad の切替）

`gbs_control.cpp` に次を追加:

- `rotaryEncoderEnabled()` — `PINCFG_USE_ROTARY_ENCODER` を参照（ビルド時定数）
- `applyInputModeFromPinConfig()`

起動時にビルド時定数で判定し、以下を切替:

- **R/E有効**: 従来どおり `pin_clk/pin_data/pin_switch` を入力+割り込み有効
- **D-Pad有効**: R/E割り込みを `detachInterrupt` して無効化し、`geometry_buttons_init()` を実行

> **設計変更メモ**: 初期実装では `options.h` の `enableRotaryEncoder` フィールドと
> `preferencesv2.txt` による実行時切替を行っていたが、upstream の `options.h` に
> 非標準フィールドを追加する必要がある点、preferencesv2 フォーマットに後方互換を
> 維持する複雑さが更新追従時のコストになるため、ビルド時定数方式に変更した。
> `options.h` は upstream からそのままコピーすれば良い。

### 4.15 main.cpp の初期化責務整理

`main/main.cpp` から `geometry_buttons_init()` の直接呼び出しを削除。

- 旧: `app_main()` が常時 Geometry 初期化
- 新: `gbs_setup()` が設定に応じて初期化

初期化順序の責務が `gbs_control.cpp` に集中し、upstream 追従時の差分把握が容易。

### 4.16 pin_config.h にモードLED再利用ピンを明示

`PIN_MODE_LED_A/B` を追加（R/Eの CLK/DATA ピンを再利用）。

- R/E有効時: エンコーダ入力として使用
- D-Pad有効時: モード表示LED出力として使用

「どのピンを再利用するか」がヘッダ1箇所に明示され、基板差分対応が簡単になる。

## 5. 新規作成ファイル（upstream に存在しない）

| ファイル | 説明 |
|---|---|
| `main/main.cpp` | ESP-IDF エントリポイント。NVS初期化 → BLE shell起動 → gbs_task作成 |
| `components/compat/` (全体) | Arduino 互換レイヤ。詳細は `docs/arduino_compat_layer.md` |
| `components/gbs_control/include/pin_config.h` | XIAO ESP32-C3/C6 用ピン定義 (自動選択) |
| `components/gbs_control/include/gbs_forward_decls.h` | .ino 自動前方宣言の代替 |
| `components/gbs_control/include/geometry_buttons.h` | D-Pad入力機能の公開ヘッダ |
| `components/gbs_control/src/geometry_buttons.cpp` | D-Pad入力、モード切替、モードLED制御 |
| `components/gbs_control/include/SSD1306Wire.h` | SSD1306 OLED ドライバ (I2C) |
| `components/gbs_control/src/SSD1306Wire.cpp` | 同上の実装 |
| `components/gbs_control/include/WebSocketServer_compat.h` | WebSocket互換インターフェース |
| `components/gbs_control/src/WebSocketServer_compat.cpp` | ESP-IDF httpd_ws ベースの実装 |
| `components/gbs_control/src/shell.cpp` | BLE シリアルシェル (設定/デバッグ) |
| `components/gbs_control/include/shell.h` | 同上のヘッダ |
| `components/gbs_control/src/shell_i2c_bridge.cpp` | Shell→メインタスク I2Cプロキシ (FreeRTOSキュー) |
| `components/gbs_control/include/shell_i2c_bridge.h` | 同上のヘッダ |
| `components/ble_serial/ble_serial.c` | NimBLE NUS サービス (BLE UART) |
| `components/ble_serial/include/ble_serial.h` | 同上のヘッダ |
| `components/gbs_presets/gbs_presets.c` | ESP-IDFコンポーネントに必要な空ソースファイル |

### 5.1 I2C バスアクセスの設計原則

ESP8266 版は単一スレッド協調マルチタスクのため、I2C バスの排他制御は不要だった。
ESP-IDF 版では FreeRTOS 上で複数タスクが並行動作するため、以下のルールを遵守する。

1. **GBS レジスタへの I2C アクセスは `tw.h` の `GBS::read/write` を使う。**
   `tw.h` の全アクセスは `lockBus()` → `setSeg()` → `rawRead/rawWrite` → `unlockBus()`
   のパターンで保護されており、Wire の再帰ミューテックスで排他制御される。
   どのタスクから呼んでも安全。

2. **`shell_i2c_bridge` は `GBS::read/write` の薄いラッパー。**
   キューやポーリングは不要。Shell タスクから直接 `shellI2cBridgeRead/Write` を呼び、
   内部で `GBS::read/write` を実行する。Wire ミューテックスが bus 排他を保証し、
   `setSeg()` の `curSeg` キャッシュも `lockBus` 区間内で一貫する。

3. **設定系コマンドは `serialCommand` / `userCommand` 変数経由** (WebUI と同じ方式)。
   直接レジスタ操作が必要な場合のみ `shellI2cBridgeRead/Write` を使う。

4. **`gbs_control.cpp` 内の `lastSegment` キャッシュ**:
   `writeBytes()` / `readFromRegister()` 用のローカルキャッシュ。
   `tw.h` 側の `curSeg` とは別物。Shell は `GBS::read/write` を使うため
   `lastSegment` には触れない。両キャッシュは独立して整合する。

5. **SSD1306 OLED やその他同一 I2C バスデバイス**: Wire ミューテックスで保護済み。
...
- [ ] 4.7 — `#if defined(ESP8266) || defined(ESP_PLATFORM)` に更新する
...
- [ ] 4.12 — コンパイラ警告になる型の修正
- [ ] 4.13 — 末尾の `#endif` 除去
- [ ] 4.14 — `applyInputModeFromPinConfig()` と `rotaryEncoderEnabled()` を再適用
- [ ] 4.15 — `main/main.cpp` の `geometry_buttons_init()` 直接呼び出しを入れない
- [ ] 4.16 — `pin_config.h` の `PIN_MODE_LED_A/B` を維持
...
```text
set input d-pad      # 保存 + 再起動、D-Pad/モードLED有効
set input r-encoder  # 保存 + 再起動、ロータリー有効
show config          # rotaryEncoder の値を確認
```

### Step 6.1: 現場PC (esptoolのみ) での書き込み

`release/esptool/` 配下に、esptool単体で書き込める最小セットを配置済み。

- `release/esptool/esp32c3/`
  - `bootloader.bin`
  - `partition-table.bin`
  - `gbs-control.bin`
  - `flash.sh` / `flash.bat`
- `release/esptool/esp32c6/`
  - `bootloader.bin`
  - `partition-table.bin`
  - `gbs-control.bin`
  - `flash.sh` / `flash.bat`
...
| モードLED A | GPIO3 | D1 | D-Padモード時のみ使用 (R/E無効時) |
| モードLED B | GPIO4 | D2 | D-Padモード時のみ使用 (R/E無効時) |
| ジオメトリボタン UP | GPIO10 | D10 | Active LOW |
| ジオメトリボタン DOWN | GPIO9 | D9 | Active LOW (strapping pin) |
| ジオメトリボタン LEFT | GPIO8 | D8 | Active LOW (strapping pin) |
| ジオメトリボタン RIGHT | GPIO20 | D7 | Active LOW |
| デバッグ入力 | GPIO21 | D6/TX | DEBUG_IN_PIN |
| LED | なし | — | このプロジェクトでは C3 の LED は未使用 |
...
| モードLED A | GPIO1 | D1 | D-Padモード時のみ使用 (R/E無効時) |
| モードLED B | GPIO2 | D2 | D-Padモード時のみ使用 (R/E無効時) |
| デバッグ入力 | GPIO16 | D6/TX | DEBUG_IN_PIN |
