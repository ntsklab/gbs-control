...
| `components/gbs_control/include/pin_config.h` | ~150 | XIAO ESP32-C3 / ESP32-C6 用ピン定義 (自動選択) |
...
### Step 6.1: esptool 配布バンドルの作成

開発環境をもたない現場PC での書き込みには、esptool 単体バンドルを使用する。

VS Code タスク **"Make Release Bundle (C3+C6)"** を実行するか、手動で次のコマンドを実行:

```bash
# C3 と C6 を一括ビルドして release/ 以下に配置
idf.py -B build-c3 -DSDKCONFIG=sdkconfig.esp32c3 build
idf.py -B build-c6 -DSDKCONFIG=sdkconfig.esp32c6 build
```

生成される配布物 (`release/esptool/<target>/`):

| ファイル | 説明 |
|---|---|
| `bootloader.bin` | ブートローダ |
| `partition-table.bin` | パーティションテーブル |
| `gbs-control.bin` | アプリケーション |
| `flash_args.txt` | esptool 書き込み引数 |
| `flasher_args.json` | idf.py 書き込み設定 |
...
esptool.py --chip esp32c3 --port /dev/ttyUSB0 --baud 460800 \
  write_flash @flash_args.txt

# Windows
esptool.exe --chip esp32c3 --port COM3 --baud 460800 write_flash @flash_args.txt
```
...
`pin_config.h` で定義 (`CONFIG_IDF_TARGET_ESP32C3`):
...
`pin_config.h` で定義 (`CONFIG_IDF_TARGET_ESP32C6`):
