...
| `options.h` | `components/gbs_control/include/options.h` | **変更あり** (`enableRotaryEncoder` 追加) |
...
upstream の `gbs-control.ino` → `gbs_control.cpp` に適用する変更は、初期移植分に加えて
入力モード切替（R/E ⇔ D-Pad）対応が入っている。以下に全変更をカテゴリ別に示す。
...
- `INPUT_MODE_ROTARY` / `INPUT_MODE_GEOMETRY` 定数
- `rotaryEncoderEnabled()`
...
これにより、アップデート時も入力制御の分岐点が1箇所に集約される。

### 4.15 userCommand 拡張（入力モード切替コマンド）

`handleType2Command()` に以下を追加:

- `case 'I'`: `enableRotaryEncoder=1`（R/E）を保存して再起動
- `case 'J'`: `enableRotaryEncoder=0`（D-Pad）を保存して再起動

Shell/WebUIから同じ経路で制御できるため、追加UI実装時も再利用しやすい。

### 4.16 preferencesv2 拡張（後方互換付き）

`userOptions` に `enableRotaryEncoder` を追加し、`preferencesv2.txt` の末尾1バイトに保存。

- 保存: `saveUserPrefs()` で `f.write(uopt->enableRotaryEncoder + '0')`
- 読み込み: EOFなら旧フォーマットとして `INPUT_MODE_ROTARY` を既定値に採用

これにより、既存環境の設定ファイルを壊さずに更新可能。

### 4.17 Shell の入力モード操作を固定語彙化

`shell.cpp` に以下を追加・更新:

- `set input <r-encoder|d-pad>`
- Tab補完候補: `r-encoder`, `d-pad`
- `show config` に `rotaryEncoder` 表示

### 4.18 main.cpp の初期化責務整理
...
### 4.19 pin_config.h にモードLED再利用ピンを明示
...
# options.h は upstream コピー後に enableRotaryEncoder を再追加
...
- [ ] 4.12 — コンパイラ警告になる型の修正
- [ ] 4.13 — 末尾の `#endif` 除去
- [ ] 4.14 — `applyInputModeFromPrefs()` と入力モード定数を再適用
- [ ] 4.15 — `handleType2Command()` の `I`/`J` を再適用
- [ ] 4.16 — `preferencesv2.txt` 読み書きに `enableRotaryEncoder` を追加（EOF後方互換込み）
- [ ] 4.17 — `shell.cpp` の `set input <r-encoder|d-pad>` とTab補完を再適用
- [ ] 4.18 — `main/main.cpp` の `geometry_buttons_init()` 直接呼び出しを入れない
- [ ] 4.19 — `pin_config.h` の `PIN_MODE_LED_A/B` を維持
