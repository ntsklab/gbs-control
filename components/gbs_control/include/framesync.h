...
// ESP-IDF port: use gpio_get_level for digitalRead
#include "Arduino.h"
...
// Forward declaration for handleWiFi (defined in gbs_control.cpp)
extern void handleWiFi(boolean instant);
...
namespace MeasurePeriod {
    ...
    void ICACHE_RAM_ATTR _risingEdgeISR_measure()
    {
        ...
        stopTime = esp_cpu_get_cycle_count();
        detachInterrupt(DEBUG_IN_PIN);
        interrupts();
    }
}
